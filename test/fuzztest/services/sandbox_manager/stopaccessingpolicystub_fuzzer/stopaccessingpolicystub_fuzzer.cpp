...
#include "stopaccessingpolicystub_fuzzer.h"
...
namespace OHOS {
    bool StopAccessingPolicyStub(const uint8_t *data, size_t size)
    {
        ...
        uint32_t code = static_cast<uint32_t>(SandboxManagerInterfaceCode::STOP_ACCESSING_URI);
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::StopAccessingPolicyStubFuzzTest(data, size);
    return 0;
}