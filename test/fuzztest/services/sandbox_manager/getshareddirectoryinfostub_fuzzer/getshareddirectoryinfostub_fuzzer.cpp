...
#include "getshareddirectoryinfostub_fuzzer.h"
...
namespace OHOS {
    ...
    bool GetSharedDirectoryInfoStubFuzzTest(const uint8_t *data, size_t size)
    {
        ...
        uint32_t code = 0xffc6;  // COMMAND_GET_SHARED_DIRECTORY_INFO from IDL
...
{
    OHOS::GetSharedDirectoryInfoStubFuzzTest(data, size);
    return 0;
}
