...
namespace {
static constexpr OHOS::HiviewDFX::HiLogLabel LABEL = {LOG_CORE, ACCESSCONTROL_DOMAIN_SANDBOXMANAGER, "Statement"};
}
...
Statement::Statement(sqlite3* db, const std::string &sql) : db_(db), sql_(sql)
{
    if (sqlite3_prepare_v2(db, sql.c_str(), sql.size(), &statement_, nullptr) != SQLITE_OK) {
        SANDBOXMANAGER_LOG_ERROR(LABEL, "Cannot prepare, errorMsg: %{public}s", sqlite3_errmsg(db_));
    }
}
...
void Statement::Bind(const int32_t index, const std::string &text)
{
    ...
    if (sqlite3_bind_text(statement_, index, text.c_str(), text.size(), SQLITE_TRANSIENT) != SQLITE_OK) {
        SANDBOXMANAGER_LOG_ERROR(LABEL, "Cannot bind string, errorMsg: %{public}s", sqlite3_errmsg(db_));
    }
}
...
void Statement::Bind(const int32_t index, int32_t value)
{
    ...
    if (sqlite3_bind_int(statement_, index, value) != SQLITE_OK) {
        SANDBOXMANAGER_LOG_ERROR(LABEL, "Cannot bind int32_t, errorMsg: %{public}s", sqlite3_errmsg(db_));
    }
}
...
void Statement::Bind(const int32_t index, int64_t value)
{
    ...
    if (sqlite3_bind_int64(statement_, index, value) != SQLITE_OK) {
        SANDBOXMANAGER_LOG_ERROR(LABEL, "Cannot bind int64_t, errorMsg: %{public}s", sqlite3_errmsg(db_));
    }
}
