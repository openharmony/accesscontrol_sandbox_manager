...
namespace {
static constexpr OHOS::HiviewDFX::HiLogLabel LABEL = {LOG_CORE, ACCESSCONTROL_DOMAIN_SANDBOXMANAGER, "SqliteHelper"};
}
...
{
    if (db_ != nullptr) {
        SANDBOXMANAGER_LOG_WARN(LABEL, "Db s already open");
        return;
    }
    ...
    if (res != SQLITE_OK) {
        SANDBOXMANAGER_LOG_ERROR(LABEL, "Failed to open db: %{public}s", sqlite3_errmsg(db_));
        return;
    }
...
{
    if (db_ == nullptr) {
        SANDBOXMANAGER_LOG_WARN(LABEL, "Do open data base first!");
        return;
    }
    int32_t ret = sqlite3_close(db_);
    if (ret != SQLITE_OK) {
        SANDBOXMANAGER_LOG_WARN(LABEL, "Sqlite3_close error, ret=%{public}d", ret);
        return;
    }
    db_ = nullptr;
}
...
int32_t SqliteHelper::BeginTransaction() const
{
    ...
    if (ret != SQLITE_OK) {
        SANDBOXMANAGER_LOG_ERROR(LABEL, "Failed, errorMsg: %{public}s", errorMessage);
...
int32_t SqliteHelper::GetVersion() const
{
    ...
    SANDBOXMANAGER_LOG_INFO(LABEL, "Version: %{public}d", version);
    return version;
}
