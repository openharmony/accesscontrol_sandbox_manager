...
{
    std::map<std::string, uint64_t> dbResultsMap;
    for (size_t i = 0; i < dbResultsSize; ++i) {
        std::string currPath = dbResults[i].GetString(PolicyFiledConst::FIELD_PATH);
        uint64_t currMode = static_cast<uint64_t>(dbResults[i].GetInt(PolicyFiledConst::FIELD_MODE));
    ...
    for (size_t i = 0; i < dbResultsSize; ++i) {
        ...
        dbResults[i].Put(PolicyFiledConst::FIELD_MODE, static_cast<int64_t>(dbResultsMap[polisyPath]));
    }
}
