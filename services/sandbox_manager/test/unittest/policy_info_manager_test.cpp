...
HWTEST_F(PolicyInfoManagerTest, PolicyInfoManagerTest008, TestSize.Level0)
{
    ...
    int32_t userId = 100;
    policy.path = path1;
    EXPECT_EQ(SandboxRetType::INVALID_PATH, PolicyInfoManager::GetInstance().CheckPathIsBlocked(userId, policy));
    policy.path = path2;
    EXPECT_EQ(SandboxRetType::INVALID_PATH, PolicyInfoManager::GetInstance().CheckPathIsBlocked(userId, policy));
    policy.path = path3;
    EXPECT_EQ(SandboxRetType::INVALID_PATH, PolicyInfoManager::GetInstance().CheckPathIsBlocked(userId, policy));
    ...
    EXPECT_EQ(SandboxRetType::INVALID_PATH, PolicyInfoManager::GetInstance().CheckPathIsBlocked(userId, policy));
    policy.path = path7;
    EXPECT_EQ(SANDBOX_MANAGER_OK, PolicyInfoManager::GetInstance().CheckPathIsBlocked(userId, policy));
    policy.path = path8;
    EXPECT_EQ(SandboxRetType::INVALID_PATH, PolicyInfoManager::GetInstance().CheckPathIsBlocked(userId, policy));
}
...
HWTEST_F(PolicyInfoManagerTest, PolicyInfoManagerTest009, TestSize.Level0)
{
    ...
    EXPECT_EQ(SandboxRetType::INVALID_PATH,
        PolicyInfoManager::GetInstance().CheckPathIsBlocked(userId, policy));
    policy.path = path2;
    policy.type = PolicyType::SELF_PATH;
    EXPECT_EQ(SandboxRetType::INVALID_PATH,
        PolicyInfoManager::GetInstance().CheckPathIsBlocked(userId, policy));
    ...
    EXPECT_EQ(SANDBOX_MANAGER_OK,
        PolicyInfoManager::GetInstance().CheckPathIsBlocked(userId, policy, "com.test"));
    policy.path = path4;
    policy.type = PolicyType::UNKNOWN;
    EXPECT_EQ(SANDBOX_MANAGER_OK,
        PolicyInfoManager::GetInstance().CheckPathIsBlocked(userId, policy, ""));
    ...
    policy.type = PolicyType::SELF_PATH;
    EXPECT_EQ(SandboxRetType::INVALID_PATH,
        PolicyInfoManager::GetInstance().CheckPathIsBlocked(userId, policy, "com.testt"));
    policy.path = path6;
    policy.type = PolicyType::SELF_PATH;
    EXPECT_EQ(SandboxRetType::INVALID_PATH,
        PolicyInfoManager::GetInstance().CheckPathIsBlocked(userId, policy, ""));
...
HWTEST_F(PolicyInfoManagerTest, PolicyInfoManagerTest012, TestSize.Level0)
{
    ...
    EXPECT_EQ(SANDBOX_MANAGER_OK, PolicyInfoManager::GetInstance().StartAccessingByTokenId(selfTokenId_, 0));

    u32Res.resize(0);
    EXPECT_EQ(SANDBOX_MANAGER_OK,
        PolicyInfoManager::GetInstance().StartAccessingPolicy(selfTokenId_, policy, u32Res, 0));
    ASSERT_EQ(1, u32Res.size());
    EXPECT_EQ(SandboxRetType::OPERATE_SUCCESSFULLY, u32Res[0]);
...
HWTEST_F(PolicyInfoManagerTest, ShareTest010, TestSize.Level0)
{
    ...
    const std::string bundleName = "com.testshare";
    SandboxManagerShare::GetInstance().DeleteByBundleName(bundleName);
    const std::string path = "/storage/Users/currentUser/appdata/el2/base/com.testshare/preferences";
    EXPECT_EQ(SANDBOX_MANAGER_OK, SandboxManagerShare::GetInstance().TransAndSetToMap(stringJson1, bundleName, userId));
...
HWTEST_F(PolicyInfoManagerTest, ShareTest012, TestSize.Level0)
{
    ...
    info.path = "/storage/Users/currentUser/appdata/el2/base/com.testshare/preferences/child";
    info.mode = OperateMode::WRITE_MODE;
    policy[0] = info;
    std::vector<uint32_t> setResult;
    SetInfo setInfo;
...
HWTEST_F(PolicyInfoManagerTest, ShareTest015, TestSize.Level0)
{
    ...
    info.path = "/storage/Users/currentUser/appdata/el2/base/+clone-1+com.testshare/haps";
    info.mode = OperateMode::WRITE_MODE;
    policy[0] = info;
    std::vector<uint32_t> setResult;
    SetInfo setInfo;
...
HWTEST_F(PolicyInfoManagerTest, ShareTest028, TestSize.Level0)
{
    ...
    std::string stringJson1 = R"({"share_files":{"scopes":[{"path": "/el4/tmp", "permission": "r"}]}})";

    SandboxManagerShare::GetInstance().DeleteByBundleName(bundleName);
    EXPECT_EQ(SANDBOX_MANAGER_OK, SandboxManagerShare::GetInstance().TransAndSetToMap(stringJson1, bundleName, userId));
    const std::string path1 = "/storage/Users/currentUser/appdata/el4/tmp/com.testshare.dist";
    EXPECT_EQ(OperateMode::READ_MODE, SandboxManagerShare::GetInstance().FindPermission(bundleName, userId, path1));
    const std::string path2 = "/storage/Users/currentUser/appdata/el4/tmp/com.testshare.dist/data";
    EXPECT_EQ(OperateMode::READ_MODE, SandboxManagerShare::GetInstance().FindPermission(bundleName, userId, path2));
...
/**
 * @tc.name: ShareTest041
 * @tc.desc: set sharemap /el2/path1, then setpolciy /el2/path2 failed
 * @tc.type: FUNC
 * @tc.require:
 */
HWTEST_F(PolicyInfoManagerTest, ShareTest041, TestSize.Level0)
{
    int32_t userId = 100;
    const std::string bundleName = "com.testshare";
    SandboxManagerShare::GetInstance().DeleteByBundleName(bundleName);
    ...
    std::vector<PolicyInfo> policy;
    policy.emplace_back(info);
    info.path = "/storage/Users/currentUser/appdata/el2/test1/com.testshare";
    info.mode = OperateMode::WRITE_MODE;
    policy[0] = info;
    std::vector<uint32_t> setResult;
    SetInfo setInfo;
    ...
    std::string stringJson = R"({"share_files":{"scopes":[{"path": "/el2/test2", "permission": "r+w"}]}})";
    EXPECT_EQ(SANDBOX_MANAGER_OK, SandboxManagerShare::GetInstance().TransAndSetToMap(stringJson, bundleName, userId));
    info.path = "/storage/Users/currentUser/appdata/el2/test2/com.testshare";
    info.mode = OperateMode::WRITE_MODE;
    policy[0] = info;
    EXPECT_EQ(SANDBOX_MANAGER_OK,
        PolicyInfoManager::GetInstance().SetPolicy(g_mockToken, policy, 1, setResult, setInfo));
    ...
    info.path = "/storage/Users/currentUser/appdata/el2/test3/com.testshare";
    info.mode = OperateMode::WRITE_MODE;
    policy[0] = info;
    EXPECT_EQ(SANDBOX_MANAGER_OK,
        PolicyInfoManager::GetInstance().SetPolicy(g_mockToken, policy, 1, setResult, setInfo));
